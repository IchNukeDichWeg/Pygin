...
class UCIEngine:
    ...
    def _ensure_pool(self):
        ...
        if self.threads > 1 and getattr(self.engine, "_smp_pool", None) is None:
            try:
                ...
                # Z-01: size the shared TT from the UCI Hash option (16 bytes
                # per slot, largest power of two that fits; floor 1 MB) --
                # it used to be pinned at the 64 MB default no matter what
                # the GUI configured.
                slots = 1 << max(16, (self.hash_mb * (1 << 20) // 16).bit_length() - 1)
                self.engine._smp_pool = SMPPool(self.threads, n_slots=slots)
    ...
    def _set_option(self, args):
        ...
        if key == "hash":
            try:
                ...
                return
            # Z-01: the shared SMP TT is sized from Hash at pool creation, so
            # a size change needs a fresh pool (recreated at next isready/go).
            pool = getattr(self.engine, "_smp_pool", None)
            if pool is not None:
                pool.close()
                self.engine._smp_pool = None
    ...
    def _apply_options(self):
        ...
        # Z-03: TT values were computed under the old weights -- keeping them
        # would poison subsequent searches (and silently corrupt tuning
        # sweeps). Option changes happen between games in practice, so the
        # cost is nil. (_eval_memo is already cleared inside _sync_c_params;
        # the SMP shared TT is cleared by the pool on config change.)
        self.engine.tt = {}
...
def main():
    ...
    for line in sys.stdin:
        ...
        try:
            ...
            # Z-02: malformed input (e.g. `go movetime abc`) must never kill
            # the process mid-game -- that's an instant forfeit. Log, ignore
            # the command, keep serving. (`quit` still exits via the break.)
            import traceback
            sys.stderr.write(f"[uci] WARNING: error handling {line!r}:\n"
