"""
engine50.py -- FROZEN SNAPSHOT of v50 (2026-07-18).
===========================================================================

Snapshot of the repo-root ``cengine.py`` at the v50 milestone: v49 + the
FI-53/FI-54 store/probe-policy pair (abi 17). FI-53 rule50 TT staleness
guard (TT_R50 / set_tt_r50): at hmc >= 90, decisive-but-non-mate TT values
(|v| >= 500cp) neither cut nor narrow at the negamax and qsearch probe
blocks -- the promised win may not be convertible before the rule draw;
mates and quiet values still cut. FI-54 depth-independent TT mate handling
(TERM_STORE + TT_MATE_CUT / set_term_store + set_tt_mate_cut): terminal
mate/stalemate returns write permanent TT_EXACT entries at sentinel depth
200, and a second negamax probe arm cuts mate-range values at any stored
depth (bound-proven, PV nodes skipped; GHI = Stockfish's accepted
tradeoff, probe arm only). KEPT-ON-NULL vs Old Engine/49: **+1.60 +/-6.8
@ 10,000 games @ 50+0.20** (50.23%, ptnml 278/1155/2106/1165/296, pair
ratio 1.02, GSPRT LLR +0.117) -- the seventh and eighth correctness
releases of the class; first campaign on rotated SUBSET_SEED 50. Paired
matetrack had leaned positive (905/777 vs 893/768). All three toggles
False restores v49 node-exact. Self-contained: the eval oracle / book
provider is the sibling ``engine_eval.py`` (the same-day engine.py
frozen), loaded by explicit path. C sources frozen alongside; ./setup.sh
builds this directory's .so files.
