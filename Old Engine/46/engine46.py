"""
engine46.py -- FROZEN SNAPSHOT of v46 (2026-07-13).
===========================================================================

Snapshot of the repo-root ``cengine.py`` at the v46 milestone: v45 with the
transposition table doubled to 22 bits (96 MB, from v45's 48 MB). Motivated
by a live hashfull capture -- a single deep search fills ~half the 48 MB
table and the warm persistent TT then climbs to 950 permille+ within a
game. A/B vs Old Engine/45 at the full 223-worker load: **+5.94 +/-6.8 over
10,000 games @ 50+0.20** (50.85%, ptnml 264/1157/2014/1274/291, pair ratio
1.10, normalized +12.33) -- a BORDERLINE-positive (the 95%% CI just touches
zero) SHIPPED on the monotonic-low-risk rationale: a bigger table cannot
worsen decision quality at fixed nodes, its only downside vector is DRAM
bandwidth, and that was exercised at full load = net positive. TT_BITS=21
restores v45's 48 MB table exactly. Self-contained: the eval oracle / book
provider is the sibling ``engine_eval.py`` (the same-day engine.py frozen),
loaded by explicit path. C sources frozen alongside; ./setup.sh builds this
