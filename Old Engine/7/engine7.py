...
* **Pruning / selectivity**: null-move pruning, reverse-futility (static
  ...
  reductions (LMR -- a log(depth)*log(move) reduction table).
* **Move ordering**: TT move, MVV-LVA captures, promotions, killer moves, the
  counter-move heuristic and the history heuristic, with **Static Exchange
  Evaluation (SEE)** demoting losing captures and pruning them in quiescence.
...
class Engine:
    ...
    MAX_EXTENSIONS = 8                       # non-check extension plies per line
    ...
    def __init__(self):
        ...
        # fixed 1/2-ply scheme is used. Benchmarked at ~-32% nodes for the same
        # fixed depth with no tactical regression (bench_search.py), so it is on
        # by default; the flag stays for A/B comparisons.
        self.lmr_aggressive = True
        self._lmr_table = [[0] * 64 for _ in range(64)]
        for d in range(1, 64):
            for m in range(1, 64):
                self._lmr_table[d][m] = int(0.75 + math.log(d) * math.log(m) / 2.25)
        ...
        # Benchmarked (bench_search.py): the quiescence search already resolves
    ...
    def _evaluate_static(self, board):
        ...
        endgame = phase <= 6
        # "Mating" scenario: one side is down to a lone king (+ pawns) while the
        # other keeps real material (K+Q vs K, K+Q+B vs K, K+Q+Q vs K, ...).
        lone_loser = ((occ_w & ~kings & ~pawns) == 0) != ((occ_b & ~kings & ~pawns) == 0)
        if lone_loser:
            # Dedicated mating evaluation. The bare-king side is lost, so the
            # only thing that matters is cornering it fast. The normal positional
            # terms (mobility / pawn structure / bishop pair / king safety) only
            # add noise that competes with the mating gradient -- that noise is
            # exactly what made the engine shuffle a winning K+Q+B vs K into a
            # repetition draw -- so skip them and use a *strong* mop-up. Material
            # + PST (already in ``score``) still dominate; the PST also helps,
            # since a bare king's endgame king-table is worst in the corners.
            ...
            if self.use_pin_eval:
                score += self._pin_penalty_bb(board, occ_w, occ_b)
            if endgame:
                # Endgame with pieces on both sides: the mop-up still helps drive
                # won material edges (KR vs KN, etc.) and self-gates otherwise.
                score += self._mopup_bb(occ_w, occ_b, knights, bishops,
                                        rooks, queens, kings)
            else:
    ...
    def _see(self, board, move):
        ...
        if board.is_en_passant(move):
            ...
            target_value = values[victim.piece_type]
            ep_sq = None

        attacker = board.piece_at(from_sq)
        if attacker is None:
            return 0
        attacker_value = values[attacker.piece_type]
