...
* **v23**: fixed a Zobrist method-swap bug -- a permanently-off ``if`` guard
  ...
  effect (within noise).
* **v24** (current ``engine.py``): the same fix applied to the TT dispatch
...
Highlights: setoption changes now reach the C eval (_sync_c_params, P-06);
UCI `stop` race closed via the host-owned `_abort` flag (P-05); zeitnot time
management -- emergency budgets respect overhead and sub-250 ms budgets bind
a 1024-node poll (P-08); all search timing on monotonic perf_counter (P-09);
TT entry cap enforced inside long searches (P-15); eval_c/movegen .so ABI
handshake + loud fallback (P-12); C-side division/ctzll(0) guards
(C-01/C-02); and the SMP cluster -- uci.py now creates the worker pool on
the main thread so production play finally runs multi-core with a live
`Threads` option (P-01), stm-relative result tie-break (P-02, the old one
picked Black's WORST tie), dead-worker timeouts + error rows (P-03),
search-id-tagged results (P-04), pool carried across ucinewgame (P-07),
host config replicated into workers (P-11), workers close their shm view
(P-13), shared-TT PV walk fixed (X-01). Single-thread search behavior is
byte-identical to v24 (h1h8/3495 reference) apart from the intended fixes
(zeitnot budgets, in-search TT cap).

Strength (absolute, vs Stockfish)
----------------------------------
Benchmarked against ``stockfish_engine.py`` at Stockfish skill ~2400, engine
running ``SMP_WORKERS = 4``: 400 games, 188W / 131D / 81L (63.4%) ->
**+95 +/-37 Elo** at 95% CI (4h25m, ~40s/game). This sits on top of v16/v17's
C-eval/C-movegen work (+69 Elo) and v19's optimization-reference work above
-- treat ~2400 (Stockfish's configured strength) as the current floor. An
older 6000-game baseline (~2341 vs Stockfish 2350) predates the C work and
ran single-threaded, so it's no longer comparable.
...
class Engine:
    ...
    def _search(self, board, max_depth, time_limit):
        ...
        self._check_time = (self._check_time_zeitnot
                            if time_limit is not None and time_limit < 0.25
