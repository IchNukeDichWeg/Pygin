"""
engine55.py -- FROZEN SNAPSHOT of v55 (2026-07-25).
===========================================================================

Snapshot of cengine.py at the v55 milestone. v55 is the C era's first
SPEED-ONLY release: FI-11 pin-aware legality and FI-42 the (mg,eg,phase)
accumulator on Board, both in csearch.c, both node-identical. The search
plays exactly the moves v54 played -- bench signature 1,461,732 UNCHANGED,
perft --deep 1,494,874,546 nodes clean, ladder d14 node-exact, accumulator
differential 0 mismatches over ~117M nodes -- it simply reaches them faster:
...
def _load_pyengine():
    """Load the FROZEN sibling engine_eval.py by explicit path -- never the
    live repo-root engine.py. v55 changed no eval value, but the freeze is
    unconditional: a later engine.py retune must not reach back into a
    shipped snapshot and silently change what it plays."""
    import importlib.util
    name = "_v55_engine_eval"
