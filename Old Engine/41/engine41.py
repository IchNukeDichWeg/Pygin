"""
engine41.py -- FROZEN SNAPSHOT of v41 (2026-07-11).
===========================================================================

Snapshot of the repo-root ``cengine.py`` at the v41 milestone: v40 + CB-02,
correctness batch #4 -- (a) the null-move TT store obeys the replacement
policy (never clobbers deeper entries, keeps a same-key entry's move);
(b) qsearch applies the 50-move rule; (c) deep null cutoffs (depth >= 10)
are verified with a reduced no-null re-search (zugzwang insurance);
(d) root fail-high moves are adopted and promoted across aspiration calls
(v30's _partial_root_move rule). A/B vs Old Engine/40: **-2.88 +/-6.8 over
10,000 games @ 50+0.20** (49.59%, ptnml 287/1198/2086/1169/260, pair ratio
0.96, normalized -6.04) -- a null KEPT as correctness (PV-02/CB-01/EP-01
precedent). The CW-01 cannot-win eval clamp is present but DORMANT
(CANTWIN=False) -- it is the tenth campaign's candidate, not part of v41.
