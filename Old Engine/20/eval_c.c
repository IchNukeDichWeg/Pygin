    ...
    int phase)
{
     ...
     * mobility_king_safety is only called at high phase (the Python
     * dispatcher's `else` branch). The original Python flow at that phase
     * always added _rook_files_bb + _bishop_pair_bb on top of the C call,
     * so we can fold them in here unconditionally and have the Python side
     * skip those two calls. Removes a second / third Python function call
     * per eval at no extra ctypes round-trip -- the gain that the standalone
     * positional_extras (kept above for completeness) couldn't capture.
     */
    {
        /* rook (semi-)open files. */
        for (t = rooks & occ_w; t; t &= t-1) {
            int rsq = __builtin_ctzll(t);
            uint64_t fmask = 0x0101010101010101ULL << (rsq & 7);
            if (!(wp & fmask))
