...
Quick-wins layered on top of the v19 measurement above (v20, not yet
Elo-measured -- each lives behind its own toggle so they're A/B-able
individually): §2.5b folded rook_files + bishop_pair INTO the existing
``mobility_king_safety`` C call, eliminating the second ctypes round-
trip that defeated standalone §2.5 (single-thread NPS +6-11 % on
...
shows up as +1 effective ply on midgame positions and is expected to
convert to Elo in a self-play match (untested as of this write).
