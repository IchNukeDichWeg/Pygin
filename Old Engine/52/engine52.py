"""
engine52.py -- FROZEN SNAPSHOT of v52 (2026-07-21).
===========================================================================

Snapshot of the repo-root ``cengine.py`` at the v52 milestone: v51 +
FI-24(a)+(b), the null-move refinement batch (abi 22). (a) NULL_NODOUBLE:
no null-after-null -- the parent's null is visible as the
prev12==0xFFFFFFFF sentinel, and two stand-pats in a row prove nothing
while hiding zugzwang two plies deep. (b) NULL_EVALR: the null reduction
grows with the eval margin, R += (prune_eval-beta)/200 capped +2, so deep
nulls fire only at clearly-winning nodes and the shallow-null population
is untouched. CONFIRMED vs Old Engine/51 -- the C era's THIRD SPRT accept
and the first verdict decided on the --nodes instrument: split 2k screen
+15.82, two 5k tranche halves (+4.79 / +10.84), **pooled 12,000 games
+6.63 +/-4.5** (ptnml 257/1376/2490/1558/319, ratio 1.15, pooled
GSPRT[0,4] LLR +4.533). Both toggles False restores v51 node-exact. This
tree also carries real UCI pondering (go ponder/ponderhit, host layer)
and the dormant FI-15 NNUE infrastructure (USE_NNUE=False, byte-exact).
