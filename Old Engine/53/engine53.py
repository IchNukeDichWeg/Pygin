"""
engine53.py -- FROZEN SNAPSHOT of v53 (2026-07-22).
===========================================================================

Snapshot of the repo-root ``cengine.py`` at the v53 milestone. Unlike every
snapshot before it, v53 changed NO C and NO code in this file: it is the
**Texel eval retune**, 44 eval scalars refitted in engine.py -- which this
driver pushes into csearch.so at construction as the eval-param oracle. So
the frozen ``engine_eval.py`` beside this file IS the release.

Fitted by texel.py on 4,000,000 quiet positions drawn from this project's
own near-equal self-play logs and labelled with the GAME RESULT (the
v30-era tune.py fit Stockfish depth-12 scores instead, i.e. SF's eval).
CONFIRMED vs Old Engine/52 on the --nodes 1.75M instrument: 1k screen
+35.39, then two 2.5k halves on separate servers (+39.08 / +36.83);
**pooled 12,000 games +37.52 +/-6.3** (55.38%, ptnml 245/1133/2264/
1802/556, ratio 1.71, GSPRT[0,2] LLR +9.918 ACCEPT) -- the fourth SPRT
accept, 3.4x the bound, and by a wide margin the largest single release in
the C-era ledger. Matetrack flat at 927/793. The eval lane's first win.
...
def _load_pyengine():
    ...
    module name -- never the live repo-root engine.py. This matters more for
    v53 than for any previous snapshot: v53 IS an engine.py change, so a
    snapshot that imported the live engine.py would silently track every
    future eval retune instead of staying frozen."""
    import importlib.util
    name = "_v53_engine_eval"
