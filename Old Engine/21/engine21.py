...
class Engine:
    ...
    def _eval_positional_white(self, board, ctx):
        ...
        # #2.5b: rook_files + bishop_pair are returned by the C
        # mobility_king_safety call at high phase. Skip the Python versions
        # there to avoid double-counting; keep them on the low-phase /
        # Python-fallback paths where the C call isn't made.
        high_phase_c = _USE_C_EVAL and phase > 6
        if not high_phase_c:
            delta += self._rook_files_bb(rooks, occ_w, occ_b, wp, bp)
            delta += self._bishop_pair_bb(bishops, occ_w, occ_b, phase)
            # #3.x: rook-on-7th. At high phase with the C eval on, this is
            # folded into mobility_king_safety (and gated by R7_MG|R7_EG
            # there); on the low-phase / Python-fallback paths we add it
            # explicitly here so the term applies uniformly across paths.
        ...
        if phase <= 6:
            delta += self._mobility_bb(board, occ_w, occ_b,
                                       knights, bishops, rooks, queens, wp, bp)
        ...
        for move, raw in zip(moves, raws):
            ...
            if victim_pt != 0:                                   # capture (#2.3)
                ...
                if self.use_capt_history:
                    score += self.capt_history.get((mover_pt, move.to_square, victim_pt), 0)
                if move.promotion:
        ...
        for move, raw in zip(moves, raws):
            ...
            if (self.use_see_prune_captures and is_capture
                ...
                if (mover_pt_raw > victim_pt_raw
                        and self._see(board, move) < -depth * 100):
                    move_index += 1
                    continue
