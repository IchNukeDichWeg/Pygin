...
/* NOTE: `castling` is accepted for signature symmetry with generate_legal
 * but never read (captures/promotions only) -- engine.py passes 0 and skips
 * its per-qnode clean_castling_rights() call (C-12). If a future revision
 * starts reading it, fix the Python call site first. */
                      ...
                      uint32_t* out)
{
    ...
    /* 1. non-pawn captures (incl. king), descending from/to (#2.3 tagged) */
    uint64_t nonpawns = (b->knights | b->bishops | b->rooks | b->queens | b->kings) & own;
    for (t = nonpawns; t; t &= ~(1ULL << from)) {
        from = 63 - __builtin_clzll(t);
        uint64_t fb = 1ULL << from, att;
 ...
 * Bump together with _MOVEGEN_C_ABI in engine.py's load block whenever an
 * exported signature or the packed move-tag layout changes, so a
 * stale-but-loadable .so is rejected at load instead of silently emitting
 * wrong moves. */
int abi_version(void) { return 1; }
