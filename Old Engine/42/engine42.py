"""
engine42.py -- FROZEN SNAPSHOT of v42 (2026-07-11).
===========================================================================

Snapshot of the repo-root ``cengine.py`` at the v42 milestone: v41 + CW-01,
the cannot-win eval clamp -- the eval clamps to 0 when the side it favors
has no pawns, no rooks/queens, and at most a lone minor (or two knights):
it cannot force mate, so the position's true upper bound is a draw. Fixes
horizon draw-dodging (a lone-bishop side no longer shuffles at "+2.6" to
avoid the capture that would reveal the draw). A/B vs Old Engine/41:
**+3.27 +/-6.8 over 10,000 games @ 50+0.20** (50.47%, ptnml
257/1115/2159/1215/254, pair ratio 1.07, normalized +6.98) -- a null KEPT
as correctness, the fifth of its class (PV-02/CB-01/EP-01/CB-02
precedent). Self-contained: the eval oracle / book provider is the sibling
