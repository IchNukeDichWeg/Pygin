"""
engine33.py -- FROZEN SNAPSHOT of v33 (2026-07-09).
===========================================================================

Snapshot of the repo-root ``cengine.py`` at the v33 milestone (v32 + P-14
TT kept warm across irreversible moves, A/B'd +23.52 +/-6.8 over 10k games
vs v32; also carries the Lazy-SMP TT-poison fix and the process-wide
search lock), made self-contained: the eval oracle / book provider is the
sibling ``engine_eval.py`` (the same-day engine.py frozen), loaded by
explicit path -- a later retune of the live engine.py can never desync
this snapshot. C sources frozen alongside; ./setup.sh builds this
directory's .so files (gitignored, once per machine).
