"""
engine64.py -- FROZEN SNAPSHOT of v64 (2026-09-13).
===========================================================================

Snapshot of cengine.py at the v64 milestone. v64 = v63 plus two confirmed
changes, each measured at the shipping instrument:

  * THE BUNDLE. Four settings that were each a NULL on their own and cross
    together: LMR_DIV 200 -> 170, SEE_SCALED_K 50 -> 75,
    SOFT_STOP_STABLE_FRAC 0.40 -> 0.45, SOFT_STOP_STABLE_ITERS 2 -> 3.
    +6.19 +/- 4.1 over 11,896 games at 50+0.5 (tranche 1 ran a FULL 10,000
    games at +5.94 +/- 4.4 with no early stop; the extension crossed).
    A bundle verdict prices the BUNDLE -- no member is individually priced.

  * THE v14 NET (nnue_v14_959f679e272e.nnue). Measured ON TOP of that
    bundle, so the two are known to work together: +78.82 +/- 8.3 over
    3,004 games at 50+0.5, ACCEPT H1 at the 1,500-pair minimum, nElo
    +121.86, pair ratio 3.37. It read +65.66 on the 10+0.1 screen, so the
    longer control made it LARGER, not smaller.

Bench signature 785,901 (v63: 1,123,567). Both halves prune harder than v63;
the bench is not an Elo instrument and is recorded for identity only.
