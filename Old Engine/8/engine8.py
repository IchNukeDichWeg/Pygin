...
class Engine:
    ...
    def _eval_positional_white(self, board, ctx):
        ...
        delta = self._pawn_structure_bb(wp, bp)
        delta += self._rook_files_bb(rooks, occ_w, occ_b, wp, bp)
        delta += self._bishop_pair_bb(bishops, occ_w, occ_b)
        ...
        if phase <= 6:
            ...
            delta += self._mobility_king_safety_bb(
                board, occ_w, occ_b, knights, bishops, rooks, queens, wp, bp)
        return delta
    ...
    def _mobility_king_safety_bb(self, board, occ_w, occ_b,
        ...
        if wksq is not None:
            score += (wring & occ_w).bit_count() * self.KING_SHIELD
            score -= w_ring_att * self.KING_RING_ATTACK
            if not (wp & self._file_bb[wksq & 7]):
                score -= self.KING_OPEN_FILE
        if bksq is not None:
            score -= (bring & occ_b).bit_count() * self.KING_SHIELD
            score += b_ring_att * self.KING_RING_ATTACK
