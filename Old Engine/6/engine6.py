...
class Engine:
    ...
    def __init__(self):
        ...
        # testing, where match results are the final arbiter.
    ...
    def _evaluate_static(self, board):
        ...
        # other keeps real material. With *extra* winning material (K+Q+Q,
        # K+Q+R+R, ...) the game phase can still exceed the endgame threshold,
        # which used to leave the engine running king-safety eval with no mop-up
        # drive -- so it converted by aimlessly grabbing pawns instead of
        # cornering the bare king. Engage the mop-up whenever a side is bare,
        # regardless of phase. (Both-sides-have-pieces positions are untouched.)
        lone_loser = ((occ_w & ~kings & ~pawns) == 0) != ((occ_b & ~kings & ~pawns) == 0)
        if endgame or lone_loser:
            # Endgame / lone-king: replace king-safety (meaningless with a bare
            # king) with a mop-up term that converts won endings (KQK, KRK, KQQK,
            # KQ vs KP, ...) instead of shuffling. Guidance only (self-gates on a
            # decisive material edge and stays well under a pawn).
