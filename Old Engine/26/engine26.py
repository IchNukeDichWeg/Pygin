...
(zeitnot budgets, in-search TT cap). **A/B vs v24 (stopped early at 3,462
of 10k games, 45+0.15s clock): +2.91 +/-11.6, normalized +5.51 -- no
regression; validated together with a 1,068-game interim (+3.8) and the
SF-2450 absolute benchmark (~2442).**

Strength (absolute, vs Stockfish)
----------------------------------
Latest (v25, 2026-07-05): vs ``stockfish_engine.py`` at Stockfish Elo 2450,
...
class Engine:
    ...
    def order_moves(self, board, tt_move=None, ply=0, counter=None,
        ...
        for move, raw in zip(moves, raws):
            ...
            if victim_pt != 0:                                   # capture (#2.3)
                ...
                to = (raw >> 6) & 63
                score = self.history[_hist_key(color, frm, to)]
    ...
    def _negamax(self, board, depth, alpha, beta, ply, ext_budget, last_cap_sq,
        ...
        if prev_move is not None:
            counter = self.countermoves[prev_move.from_square
                                        | (prev_move.to_square << 6)]   # P-23
        scored = self.order_moves(board, tt_move, ply, counter, prev_move)   # P-19
        ...
        for _sc, move, raw, ordering_see in scored:    # P-19: no re-zip
            ...
            if is_quiet:
                frm = raw & 63                    # P-28: from/to from the tag
                to = (raw >> 6) & 63
                hist = hist_tab[_hist_key(color, frm, to)]
                if pm1 is not None:
                    hist += cont1_get(
