"""
engine51.py -- FROZEN SNAPSHOT of v51 (2026-07-18).
===========================================================================

Snapshot of the repo-root ``cengine.py`` at the v51 milestone: v50 + FI-56
root-move LMR (ROOT_LMR / set_root_lmr, abi 18) -- the first root
reductions in the engine's history. Late (i >= 4) quiet non-promotion root
moves that neither respond to nor give check are scouted at depth-1-R
(R = g_lmr[d][i]/2 capped at depth-2, depth >= 3), with a full-depth
zero-window verify before the full-window re-search; the PV move is never
reduced. CONFIRMED vs Old Engine/50 -- the C era's SECOND SPRT accept:
2k screen +17.56 +/-15.3 (CI > 0), offset-1000 main tranche ACCEPT H1 at
7,343 games (+9.37 +/-8.0, LLR +2.957, stopped early), pooled 9,343 games
**+11.12 +/-5.3** (ptnml 220/996/1988/1173/282, pair ratio 1.20, pooled
GSPRT[0,4] LLR +4.549). ROOT_LMR = False restores v50 node-exact. Also
frozen dormant: the FI-15 NNUE Phases 1-5 infrastructure behind USE_NNUE
(default False = byte-exact; no real net exists). Self-contained: the
eval oracle / book provider is the sibling ``engine_eval.py`` (the
