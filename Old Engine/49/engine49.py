"""
engine49.py -- FROZEN SNAPSHOT of v49 (2026-07-17).
===========================================================================

Snapshot of the repo-root ``cengine.py`` at the v49 milestone: v48 + FI-29,
cuckoo upcoming-repetition (CYCLE_DETECT, abi 13) -- an 8192-slot van
Kervinck table of reversible-move Zobrist deltas; when the side to move can
FORCE a repetition with one move, the node takes the contempt draw as an
alpha-raise a full search earlier. In-tree only, never in check,
castling-right-stripping matches rejected (key-sound beyond Stockfish's
envelope). KEPT-ON-NULL vs Old Engine/48: **+0.97 +/-6.8 @ 10,000 games @
50+0.20** (50.14%%, ptnml 280/1213/1978/1257/272, pair ratio 1.02, GSPRT
LLR -0.19) -- the sixth correctness release of its class. CYCLE_VERIFY
differential 13,272 claims / 0 mismatches; paired matetrack noise-flat.
CYCLE_DETECT = False restores v48 node-exact. Self-contained: the eval
oracle / book provider is the sibling ``engine_eval.py`` (the same-day
engine.py frozen), loaded by explicit path. C sources frozen alongside;
./setup.sh builds this directory's .so files.
