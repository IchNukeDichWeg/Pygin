"""
engine63.py -- FROZEN SNAPSHOT of v63 (2026-09-11).
===========================================================================

Snapshot of cengine.py at the v63 milestone. v63 is v62's settings on v61's C
core plus FI-38 depth-scaled SEE pruning of losing captures at K=50
(SEE_SCALED_K = 50): a capture that loses material by SEE, at depth <= 6, off
the PV, not in or giving check, is skipped when see < -50 * depth. The b05
FI-115 completion that v62 shipped after it had been rejected is reverted, so
the TT replacement rule is v61's again; the retained-TT probe reads 2,479,366
here with FI-38 off, against Old Engine/62's 3,294,864.

OPEN 1, the v62 settings on the clean core vs Old Engine/61 at 50+0.5: ACCEPT
H1, LLR +3.049 at 3,006 games, +20.95 +/- 7.9. FI-38 K=50 over that: 10+0.1
ACCEPT H1 (LLR +2.946, 7,813 games, +9.34 +/- 5.3), then 50+0.5 ACCEPT H1 (LLR
+2.961, 6,802 games over two tranches, +9.50 +/- 6.0). Every run stopped at a
bound, so every magnitude carries the stopping bias and the LEDGER STAYS AT
~+354; a fixed-budget 50+0.5 run of this snapshot vs Old Engine/61 is owed.

Bench signature 1,123,567 (was 1,203,792).
