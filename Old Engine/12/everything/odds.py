...
Run::
    ...
    python3 odds.py            # CPython
    pypy3   odds.py            # PyPy (engine.py ~1.5x faster)

Configure everything in the CONFIG block, run, watch.
"""
...
#              time_manager.calculate_move_time (uses CLOCK_MINUTES / INCREMENT)
...
# Clock state is reset to CLOCK_MINUTES at the start of every game.

ENGINE_1_MODE             = "time"
ENGINE_1_TIME_MS          = 1000
ENGINE_1_DEPTH            = 6
ENGINE_1_CLOCK_MINUTES    = 5
ENGINE_1_CLOCK_INCREMENT  = 2

ENGINE_2_MODE             = "time"
ENGINE_2_TIME_MS          = 500
ENGINE_2_DEPTH            = 6
ENGINE_2_CLOCK_MINUTES    = 5
ENGINE_2_CLOCK_INCREMENT  = 2
...
MAX_PLIES             = 200             # hard cap -> adjudicated draw
...
# ---------------------------------------------------------------------- #
class EnginePolicy:
    def __init__(self, path, mode, time_ms, depth, clock_min, clock_inc, tag):
        ...
        self.clock_minutes = float(clock_min)
    ...
    def describe(self):
        if self.mode == "time":
            return f"{self.time_ms} ms/move"
        if self.mode == "depth":
            return f"fixed depth {self.depth}"
        s = int(self.clock_minutes * 60)
        return f"clock {s // 60}:{s % 60:02d} + {self.clock_inc_ms / 1000.0:g}s"

    def reset_clock(self):
        """Initialise the clock for a fresh game (clock mode only)."""
        if self.mode == "clock":
            self.clock_ms = int(self.clock_minutes * 60000)
        else:
            self.clock_ms = None
        self.clock_started = False
...
def write_summary(fh, p1, p2, tally, total_games, start_t, stopped):
    lines = ["", "=== MATCH SUMMARY ===",
             f"Engine 1: {p1.name}  [{p1.describe()}]",
             f"Engine 2: {p2.name}  [{p2.describe()}]",
             f"Games scored: {tally['completed']}  (of {total_games} scheduled)",
             f"Engine 1 wins: {tally['e1']}",
             f"Engine 2 wins: {tally['e2']}",
             f"Draws: {tally['draws']}",
             f"Errors (excluded): {tally['errors']}"]
    if tally["completed"]:
        score = (tally["e1"] + 0.5 * tally["draws"]) / tally["completed"]
        el, margin = elo(score, tally["completed"])
        lines.append(
            f"Engine 1 score: {tally['e1'] + 0.5*tally['draws']:.1f}"
            f"/{tally['completed']} ({100*score:.1f}%)  =>  "
            f"{el:+.0f} +/- {margin:.0f} Elo")
    if stopped:
...
def main():
    ...
    try:
        p1 = EnginePolicy(ENGINE_1_PATH, ENGINE_1_MODE,
                          ENGINE_1_TIME_MS, ENGINE_1_DEPTH,
                          ENGINE_1_CLOCK_MINUTES, ENGINE_1_CLOCK_INCREMENT,
                          "engine_1")
        p2 = EnginePolicy(ENGINE_2_PATH, ENGINE_2_MODE,
                          ENGINE_2_TIME_MS, ENGINE_2_DEPTH,
                          ENGINE_2_CLOCK_MINUTES, ENGINE_2_CLOCK_INCREMENT,
                          "engine_2")
    except Exception as ex:
        print(f"  failed: {ex}")
        return

    if ODDS_GIVEN_BY == "engine_1":
        odds_giver = p1
    elif ODDS_GIVEN_BY == "engine_2":
        odds_giver = p2
    elif ODDS_GIVEN_BY == "none":
        odds_giver = None
    else:
    ...
    mate_score = int(getattr(p1.engine, "MATE_SCORE",
                             getattr(p2.engine, "MATE_SCORE", 1_000_000)))
    mate_threshold = int(getattr(p1.engine, "MATE_THRESHOLD",
                                  getattr(p2.engine, "MATE_THRESHOLD",
                                          mate_score - 1_000)))
    perspective_white = (PERSPECTIVE == "white")

    # --- Open the log + PGN files (same naming as match.py) ----------- #
    stamp = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    log_path = f"{p1.name}_vs_{p2.name}_odds_{stamp}_{os.getpid()}.txt"
    ...
    odds_desc = (f"{odds_label(apply_odds(chess.Board(), chess.WHITE, ODDS_SQUARES))} "
                 f"given by {odds_giver.name}") if odds_giver else "none"

    banner = (f"Odds match: {p1.name}  vs  {p2.name}\n"
              f"Interpreter: {interp}\n"
              f"Engine 1: {p1.describe()}\n"
              f"Engine 2: {p2.describe()}\n"
              f"Material odds: {odds_desc}\n"
              f"Games: {NUM_GAMES}   (Engine 1 plays {ENGINE_1_PLAYS_FIRST} "
              f"in Game 1, colors alternate)\n"
              f"Log: {log_path}\n"
              f"PGN: {pgn_path}\n" + "-" * 72)
    print(banner)
    if fh is not None:
        fh.write(f"{p1.name} vs {p2.name}\n"
                 f"Interpreter: {interp}\n"
                 f"Engine 1: {p1.describe()}   Engine 2: {p2.describe()}\n"
                 f"Material odds: {odds_desc}\n"
                 f"Games scheduled: {NUM_GAMES}\n"
    ...
    try:
        for round_no in range(1, NUM_GAMES + 1):
            # Alternate colors every game so each engine plays White ~half.
            ...
            g = play_game(round_no, p_white, p_black, odds_giver,
                          mate_score, mate_threshold, perspective_white)

            pgn_str = build_pgn(g, round_no, p1, p2)
            write_game_block(fh, pgn_fh, g, p1, p2, pgn_str)

            if g["error"] is not None:
                tally["errors"] += 1
                tag = f"ERR ({g['error'][:48]})"
            else:
                tally["completed"] += 1
                if g["winner"] is None:
                    tally["draws"] += 1
                    ...
                    tally["e1"] += 1
                    tag = f"{p1.name} wins  {g['reason']}"
                else:
                    tally["e2"] += 1
                    tag = f"{p2.name} wins  {g['reason']}"
            ...
            print(f"[{round_no:>4}/{NUM_GAMES}] {g['white'].name}(W) vs "
                  f"{g['black'].name}(B)  ->  {g['result']:>7}  "
                  f"{tag:<40} | {run}")
    except KeyboardInterrupt:
        stopped = True
        print("\n[interrupted -- writing summary so far]")

    write_summary(fh, p1, p2, tally, NUM_GAMES, start_t, stopped)
    if fh is not None:
        try:
            fh.close()
        except Exception:
