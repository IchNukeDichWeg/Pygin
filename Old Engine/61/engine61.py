"""
engine61.py -- FROZEN SNAPSHOT of v61 (2026-08-29).
===========================================================================

Snapshot of cengine.py at the v61 milestone. v61 ARMS FI-115 DEAD-ENTRY TT
REPLACEMENT (TT_DEADTAG True): material is irreversible, so a TT entry whose
stored piece count exceeds the current root's describes a position that can
never recur -- garbage with certainty, not a guess. It is evicted first, and a
still-reachable old entry is depth-protected instead of clobbered on age alone.
The count rides in 6 spare bits of d2 and is stamped either way, so with the
toggle off the store path is byte-identical to v60.

vs Old Engine/60 with 192 MiB on BOTH sides -- the shipped table size -- TIMED
50+0.5 on x86: +15.89 +/- 4.2 over a FIXED 10,000-game budget (52.29%, ptnml
146/1044/2226/1375/209, ratio 1.33, nElo +25.77). NO SPRT and no early stop, so
the magnitude carries no stopping bias and the ledger advances on it,
~+338 -> ~+354. Bench signature 1,140,099, UNCHANGED: bench searches from a
cold TT, so the table never fills and a replacement rule has nothing to choose
between -- the bench is blind to this change by construction.

Also carried: FI-113 (TT prefetch across both cache lines, node-identical,
+1.09% NPS). Rejected alongside: FI-116 growing TT, -2.88 +/- 4.4.
...
per snapshot. Retiring that net into "NNUE/Old NNUE/" breaks the path, and
there is no silent HCE fallback: construction raises.
