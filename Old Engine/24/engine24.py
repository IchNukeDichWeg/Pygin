...
Quick-wins layered on top of the v19 measurement above (v20, A/B verdict
recorded below -- each lives behind its own toggle so they remain
individually A/B-able): §2.5b folded rook_files + bishop_pair INTO the
existing ``mobility_king_safety`` C call, eliminating the second ctypes
round-trip that defeated standalone §2.5 (single-thread NPS +6-11 % on
opening / midgame, flat on endgames where the call isn't made). Three
Phase 3 evaluation terms followed: ``use_rook_on_7th`` (per-rook bonus
on the side's 7th when the enemy king sits on its back rank OR an enemy
pawn still sits on its 7th -- gated so it's never "rook on 7th for the
sake of it"); ``use_mobility_area`` (subtracts enemy-pawn-attacked
squares from every piece's mobility count -- a knight stepping there is
just lost); and ``use_threats`` (per enemy non-pawn attacked by one of
our pawns, plus per enemy major attacked by one of our minors). Cross-
snapshot v19 → v20 NPS is roughly flat in aggregate (~+5 %); the win
showed up as +1 effective ply on midgame positions.

v20 A/B vs v19 at 0.75+0.25 clock TC, single-thread, no book, 4 000
games against a Stockfish-derived position set: 1494 wins / 1532 draws
/ 974 losses, 2260.0/4000 (56.5 %) → +45 ± 11 Elo at 95 % CI. Wall
time 10 h 27 m / ~9.4 s per game across 9 parallel workers. Decisive
margin of 520 net wins out of 2 468 decisive games (~21 %), consistent
with eval-driven gains: draw rate (~38 %) is flat vs v19's pure-search
behaviour, but the engine now converts more winning positions and
resists more losing ones. The four features were tested as a single
batch; per-feature Elo attribution would need three more A/B matches
with one toggle flipped each time, which the toggles are designed for.

v21 adds a second easy-Elo batch on top of v20: capture history ordering
(``use_capt_history`` -- MVV-LVA blended with a learned (mover_pt, to_sq,
victim_pt) table), SEE-pruning of losing captures at depth ≤ 2
(``use_see_prune_captures``), LMR for losing captures (heavier-takes-lighter
proxy, reduction=1), and the in-check static-eval proxy (``use_check_eval_proxy``
feeds tt_eval/alpha into the improving heuristic through check evasions).
Also shipped in v21: five trivials (#7 bitboard has_non_pawn_material, #11
time-check interval 1023→4095, #12 pre-alloc SEE gain array, #14 bitboard
is_passed_pawn_push, #15 direct killer slots). v21 A/B vs v20 at 0.65+0.1 clock TC, single-thread, no book, 5 000 games
against the Stockfish-derived position set: 1628 wins / 1974 draws / 1398
losses, 2615.0/5000 (52.3 %) → +16 ± 10 Elo at 95 % CI. Wall time 11 h 38 m
/ ~8.4 s per game across 9 parallel workers. Smaller gain than v20's +45
(these are search heuristics, not new eval terms) but clearly significant at
5 000 games.
...
OPTIMIZATION CHANGELOG (2026-06-22 / 2026-06-23 / 2026-06-24 / 2026-06-25 / 2026-06-26)
--------------------------------------------------------------------------
...
#13 Lazy SMP -- multi-process parallel search (started 2026-06-25).  The one
    ...
    Phase 4: clock wiring is DONE (match.py MODE="clock" + uci.py both route
      clock budgets through get_best_move_timed -> SMP already). Remaining:
      run the dedicated A/B match (engine.py SMP_WORKERS=4 vs SMP_WORKERS=1,
      match.py clock TC) to confirm the +1-ply depth gain converts to Elo.
    Snapshot v18 = pre-Phase-2 full state.

Eval features tested post-v21 (all kept OFF by default in engine.py):
---------------------------------------------------------------------------
Each feature was isolated behind a toggle and A/B'd against the v21 base
(engine.py) at 0.65+0.1 TC, 5000 games, using match.py.  Threshold for
folding into the default: +10 Elo confirmed.

Outpost (``use_outpost``):  Knight/bishop bonus for a square supported by a
    friendly pawn and unreachable by any enemy pawn, on rank 5+ (white) /
    rank 4- (black).  Tapered MG/EG per piece type.
    A/B result: +0 ±10 Elo (5000 games, 2026-06-27). Neutral -- kept OFF.

Space (``use_space``):  Bonus for safe central squares (c-f files, ranks
    2-4 / 5-7) not attacked by an enemy pawn.  Tapered to zero in the
    endgame.
    A/B result: -9 ±14 Elo (5000 games, 2026-06-27). Negative -- kept OFF.

Phalanx / connected pawns (``use_phalanx``):  Bonus per pawn that is
    side-by-side with a friendly pawn (phalanx) or defended by one from
    behind (supported).  PHALANX_MG=10 / PHALANX_EG=5.
    A/B result: +3 ±10 Elo (5000 games, 2026-06-28, local only -- AWS run
    did not complete). Neutral -- kept OFF.

Pawn storm (``use_storm``):  Bonus for friendly pawns that have crossed the
    midline on the three files centred on the enemy king's file (ranks 5-7
    for white / ranks 2-4 for black).  Pure MG term (STORM_MG=12, EG=0).
    A/B result: -5 ±10 Elo (5000 games, 2026-06-29; base 1510W/2059D/1431W).
    Slightly negative -- kept OFF.

King shelter depth (``use_king_shelter``):  Replaces the legacy flat pawn-
    shield (``popcount(king_ring & own_pieces) * KING_SHIELD_MG``) with a
    per-file, per-distance assessment for the three files around the king.
    A pawn immediately in front (dist=1) scores SHELTER_CLOSE (8 cp); a pawn
    two ranks away (dist=2) scores SHELTER_FAR (4 cp); no pawn = no bonus.
    Tapered to 0 in the endgame (king should be active there).
    A/B result: +10 ±10 Elo (5000 games, 2026-06-29; 1589W/1965D/1446W for
    shelter).  Borderline at threshold -- included in the combined 20k run
    alongside phalanx and outpost rather than folded in individually.
    Kept OFF pending combined result.
...
were all tested as post-v21 eval extras and kept OFF -- see entries above.)
...
def _cont_hist_key(pm_from, pm_to, color, frm, to):
    """(pm_from, pm_to, color, frm, to) -> int. Same low 13 bits as _hist_key
    (to | frm<<6 | color<<12), plus the 1-ply predecessor's from/to on top."""
...
class Engine:
    ...
    def __init__(self):
        ...
        self.killer_0 = {}          # ply -> most-recent killer
        self.killer_1 = {}          # ply -> second killer
    ...
    # ------------------------------------------------------------------ #
    def _make_nozob(self, board, move):
    ...
    # to the correct pair for use_zobrist/use_shared_tt before the tree walk
    # starts, so this default only matters outside an active search.
    _make = _make_nozob
    ...
    def order_moves(self, board, tt_move=None, ply=0, counter=None,
        ...
        Returns ``(moves, raws, sees)`` -- three parallel lists, sorted
        together -- so the caller can read per-move tags from the raw uint32
        without re-querying the board (see #2.3 in this file's header), and
        reuse an already-computed SEE value (``sees[i]``, or None if this
        loop didn't compute one for that move) instead of recomputing it
        (roadmap item #14).
        ...
        """
        k0 = self.killer_0.get(ply)
        k1 = self.killer_1.get(ply)
        color = board.turn
        scored = []
        ...
        for move, raw in zip(moves, raws):
            ...
            if victim_pt != 0:                                   # capture (#2.3)
                ...
                if self.use_capt_history:
                    score += self.capt_history.get(
                        _capt_hist_key(mover_pt, move.to_square, victim_pt), 0)
                if move.promotion:
                ...
                frm = move.from_square
                to = move.to_square
                score = self.history.get(_hist_key(color, frm, to), 0)
                # Continuation history (#1.6): add 1-ply and 2-ply scores
                # for the same predecessor when one is in scope.
                if pm1 is not None:
                    score += cont1.get(_cont_hist_key(pm1[0], pm1[1], color, frm, to), 0)
                if pm2 is not None:
                    score += cont2.get(_cont_hist_key(pm2[0], pm2[1], color, frm, to), 0)
            scored.append((score, move, raw, see))
        scored.sort(key=_FIRST, reverse=True)
        return ([t[1] for t in scored], [t[2] for t in scored], [t[3] for t in scored])
    ...
    def _capture_moves(self, board):
        ...
        scored.sort(key=_FIRST, reverse=True)
        return ([move for _, move, _ in scored], [raw for _, _, raw in scored])
    ...
    def _store_killer(self, move, ply):
        if self.killer_0.get(ply) == move or self.killer_1.get(ply) == move:
            return
        self.killer_1[ply] = self.killer_0.get(ply)
    ...
    def _update_capt_history(self, mover_pt, to_sq, victim_pt, bonus):
        ...
        old = self.capt_history.get(key, 0)
    ...
    def _search(self, board, max_depth, time_limit):
        ...
        if self.use_zobrist or self.use_shared_tt:
            ...
            self._make = self._make_zob
            self._make_null = self._make_null_zob
            self._unmake = self._unmake_zob
        ...
        self._make = self._make_nozob
        self._make_null = self._make_null_nozob
        self._unmake = self._unmake_nozob
    ...
    def _search_root(self, board, depth, pv_move, alpha, beta):
        ...
        moves, raws, _sees = self.order_moves(board, pv_move, 0)
        for move, raw in zip(moves, raws):
    ...
    def _negamax(self, board, depth, alpha, beta, ply, ext_budget, last_cap_sq,
        ...
        if board.is_insufficient_material() or (
                board.halfmove_clock >= 100 and not board.is_check()):
        ...
        moves, raws, sees = self.order_moves(board, tt_move, ply, counter, prev_move)
        ...
        for move, raw, ordering_see in zip(moves, raws, sees):
            ...
            if is_quiet:
                frm = move.from_square
                to = move.to_square
                hist = self.history.get(_hist_key(color, frm, to), 0)
                if pm1 is not None:
                    hist += self.cont_history.get(
                        _cont_hist_key(pm1[0], pm1[1], color, frm, to), 0)
                if pm2 is not None:
                    hist += self.cont_history_2.get(
                        _cont_hist_key(pm2[0], pm2[1], color, frm, to), 0)
            else:
                hist = 0
            ...
            if (self.use_see_prune_captures and is_capture
                    and not is_pv and not in_check
                    and depth <= 2 and move_index > 0):
                ...
                if mover_pt_raw > victim_pt_raw:
                    ...
                    see = ordering_see if ordering_see is not None else self._see(board, move)
                    if see < -depth * 100:
                        move_index += 1
                        continue
            ...
            elif (depth >= 3 and move_index >= self.LMR_MIN_MOVE
                ...
                if is_quiet:
                    ...
                    if pm1 is not None:
                        self._update_cont_history(self.cont_history,
                            _cont_hist_key(pm1[0], pm1[1], color, rfrm, rto), bonus)
                        if self.use_history_malus:
                            for q in searched_quiets[:-1]:
                                self._update_cont_history(self.cont_history,
                                    _cont_hist_key(pm1[0], pm1[1], color,
                                                  q.from_square, q.to_square), -bonus)
                    if pm2 is not None:
                        self._update_cont_history(self.cont_history_2,
                            _cont_hist_key(pm2[0], pm2[1], color, rfrm, rto), bonus)
                        if self.use_history_malus:
                            for q in searched_quiets[:-1]:
                                self._update_cont_history(self.cont_history_2,
                                    _cont_hist_key(pm2[0], pm2[1], color,
                                                  q.from_square, q.to_square), -bonus)
                break
            move_index += 1
    ...
    def _quiescence(self, board, alpha, beta, ply):
        ...
        if in_check:
            ...
            moves, _raws, _sees = self.order_moves(board, None, ply)   # #2.3: raws/sees unused here
            if not moves:
        ...
        cap_moves, cap_raws = self._capture_moves(board)
        for move, raw in zip(cap_moves, cap_raws):
            # Per-move delta + SEE pruning on the captured material. Roadmap
            # item #10: victim_pt/mover_pt read straight from the raw tag
            # _capture_moves already computed for MVV-LVA ordering, instead of
            # re-deriving via board.is_en_passant + two piece_type_at calls
            # here (quiescence is ~50% of all nodes). raw's victim_pt already
            # correctly reads as PAWN for an en-passant capture (set at the
            # tag's construction, see _synth_raws / the C capture generator),
            # so no separate is_en_passant check is needed.
            if not move.promotion:
                victim_pt = (raw >> MV_SHIFT_VICTIM) & MV_MASK_PT
                victim_value = self.PIECE_VALUES[victim_pt] if victim_pt else 0
    ...
    def _check_time(self):
        ...
        if self.time_limit is None:
            return
        if (self.nodes & 4095) == 0 and (time.time() - self.start_time) >= self.time_limit:
            raise _TimeUp()
