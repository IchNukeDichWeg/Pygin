...
| shipped | PV-01 triangular PV (`cs_get_pv`; each PV node prepends its best move to the child's line on an in-window score — negamax, qsearch and the root all collect; the driver's `_extract_pv` emits the exact prefix in full, uncapped for mate lines, splicing the old TT walk only past a truncation, falling back to the pure walk on a fail-low/partial iteration) | **NODE-EXACT** (v36 ladder passes natively — zero search decisions read the tables); mate-in-4/5 spot check @1s: 5/6 full mate PVs (old TT walk: matetrack Bad-PV rate 50–61% at every TC) | **KEPT (reporting-only)** 2026-07-10 — matetrack Bad-PV re-run is the acceptance test; the residual truncations come from TT cutoffs at PV nodes (→ PV-02) |
| queued | PV-02 exact PV (`set_pv_exact`, default OFF; PV nodes skip the TT cutoff/bound-narrowing block — the standard strong-engine rule; the TT move still orders) | Mate spot check 6/6 full mate PVs; tree-changing: PV nodes re-search instead of cutting (PV nodes are rare, expected ~0 cost, but the warm P-14 table makes exact same-depth PV hits common enough to matter) | QUEUED for its own A/B slot 2026-07-10 |
| v37-dev | P-47 check-extension budget raise (`set_check_ext_budget`; 5 = v36 node-exact, LIVE CANDIDATE `cengine.CHECK_EXT_BUDGET = 8` — deeper forced/mating check lines, motivated by the matetrack curve) | Runtime setter verified; selftest pins the ladder to 5; extensions vein is thin per ledger (P-01 +6.8, P-43 +3.5 marginal) — tempered expectations | **PENDING A/B vs Old Engine/36** — second 50+0.20-era campaign (user request 2026-07-10) |
