...
| v35-dev | P-04 "improving" flag (`set_improving`, default on; v30's exact recipe: per-thread eval stack, improving = own static eval > two plies ago; RFP margin ×(depth−improving), frontier futility +RFP/2 when declining, LMR+1 when declining) | A/B vs v34 pending; `set_improving(0)` node-exact vs v34 (12-depth ladder); ON cuts nodes ~56% at d12, tactical suite + mate-in-2 intact, 2s probe reaches d17 (was d16) at 3.0M nps | PENDING 2026-07-09 |
