...
**Status:** SHIPPED and iterating — all 4 phases done; live = v38-dev, last snapshot Old Engine/37 (C-era ledger ≈ +135 over v31, v37 = exact-PV correctness release at ~0 Elo; see the version table below). **Started:** 2026-07-08 (baseline v30, ~90k NPS). **A/B TC:** 45+0.10 through v36, 50+0.20 from v37 campaigns.
...
| v38-dev | CB-01 correctness batch (`cengine.SCORE_HYGIENE` → `set_score_hygiene`; 7 sub-±6.8 fixes: Texel delta values, qsearch in-check repetition + insufficient material [draws before the qsearch TT probe, g_path logs qsearch keys], null fail-soft + TT LOWER store, qsearch TT alpha narrowing, mate-distance pruning at NON-PV nodes only, deep-qsearch killer slot 63) | OFF = v37 node-exact (ladder-pinned); matetrack @0.5s **868/751, ZERO Bad PVs** (v37: 692/600/0 — MDP alone ≈ +25% found); MDP@PV-nodes lesson: the fastest-mate score == clamped beta starved PV-01's in-window store (470 Bad PVs) → non-PV restriction | **PENDING A/B vs Old Engine/37** — fifth 50+0.20 campaign, KEEP-ON-NULL (correctness) 2026-07-10 |
