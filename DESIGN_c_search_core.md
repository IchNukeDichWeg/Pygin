...
**Status:** SHIPPED and iterating — all 4 phases done; live = v43-dev, last snapshot Old Engine/42 (C-era ledger ≈ +148 over v31; v37/v38/v40/v41/v42 correctness releases, v39 = +8.86 Phase-2 NPS train; see the version table below). Armed candidate: NV-01 verification isolation (v43-dev row below). **Started:** 2026-07-08 (baseline v30, ~90k NPS). **A/B TC:** 45+0.10 through v36, 50+0.20 from v37 campaigns.
...
| v43-dev | NV-01 verification isolation (`cengine.NULL_VERIFY = False` → `set_null_verify`): REMOVES CB-02(c), the deep-null verification search — the CB-02 component with the real timed cost (paired 5s startpos: verify-off reaches d18 consistently, verify-on d17/d18; the fixed-depth d12 inflation was mostly the store policy, verification recovers only −25 nodes there — it fires in DEEP budgets where depth ≥ 10 nodes abound). Modern top engines run unverified null | True = v42 node-exact (ladder-pinned); shipped default (False) diverges only at depth ≥ 10 nodes | **PENDING A/B vs Old Engine/42** — eleventh 50+0.20 campaign 2026-07-11; positive ⇒ v43 drops verification (ledger records the insurance cost), null ⇒ verification really is free, restore True |
