...
| v44-dev | FI-26a TT prefetch (`TT_PREFETCH(c.key)` after apply_move in negamax/qsearch/root; unconditional — adopted): P-45's null INVERTED by FI-01's free child key | NODE-IDENTICAL (a prefetch is a hint) — ladder passes UNCHANGED, no pin; paired bench +4.9% NPS median, 3/3 tight pairs (+4.7/+4.7/+5.8, warmup-discarded); a staged-quiet lazy pick was tried alongside, stream-identical and ladder-verified, but PARKED — the noisy-session bench couldn't separate it and its all-node worst case is memmove-heavy | **PENDING A/B vs Old Engine/43** — thirteenth 50+0.20 campaign 2026-07-12 (the P-22 rule: node-identical speed still needs timed Elo; expect ~+5 at ~1 Elo/1%) |
