...
| v35-dev | P-43 single-reply / forced-move extension (`set_single_reply`, default on; a node with one legal move gets +1 ply from its own budget=5, separate from the check budget) | A/B vs v34 pending; `set_single_reply(0)` node-exact vs v34 (12-depth ladder); cheap (forced node = width 1, deepens one line). Recapture ext deliberately skipped — v30 found it redundant with qsearch+SEE, which the C core has | PENDING 2026-07-09 |
