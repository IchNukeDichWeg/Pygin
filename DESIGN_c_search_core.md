...
| v37-dev | PV-02 exact PV (`cengine.PV_EXACT = True` → `set_pv_exact`; PV nodes skip the TT cutoff/bound-narrowing block — the standard strong-engine rule; the TT move still orders) | Verified on a failing matetrack FEN: C PV goes 1 move → full 13-ply mate PV ending in checkmate; d12 3-FEN probe −20% nodes (tree reshapes, sign unknown, no blowup); selftest pins the ladder to 0 | **PENDING A/B vs Old Engine/36** — third 50+0.20-era campaign 2026-07-10; matetrack Bad-PVs (~60%) only drop when this lands; confirm ⇒ v37 + matetrack re-baseline |
