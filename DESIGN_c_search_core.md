...
**Status:** SHIPPED and iterating — all 4 phases done; live = v44-dev, last snapshot Old Engine/43 (C-era ledger ≈ +153 over v31; v37/v38/v40-v42 correctness releases, v39 = +8.86 NPS train, v43 = +5.18 verification removal; see the version table below). Armed candidate: FI-04 history-based LMR (v44-dev row below). **Started:** 2026-07-08 (baseline v30, ~90k NPS). **A/B TC:** 45+0.10 through v36, 50+0.20 from v37 campaigns.
...
| v44-dev | FI-04 history-based LMR (`cengine.LMR_HIST = 8192` → `set_lmr_hist`): the quiet's butterfly history nudges its LMR reduction ±1 (adj = hist/div clamped; HIST_MAX 16384, div 8192 = strong-signal-only) — the v39+ wave's 5/5-consensus feature, move-quality at fixed depth (P-23's paying family), tempered by the Q-01/P-42 finer-signal nulls | 0 = v43 node-exact (ladder-pinned); INERT at ladder depths (history values stay < 8192 by d12 — mechanism verified firing at div 512: d12 828,647 → 1,107,416), fires at match depth ~17+ where hot moves saturate | **PENDING A/B vs Old Engine/43** — twelfth 50+0.20 campaign 2026-07-11; if +3..+10 marginal, ONE divisor tune (4096/12288) before the next feature |
