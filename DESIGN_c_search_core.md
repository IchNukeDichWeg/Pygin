...
| v41-dev | FI-08/Q-03 qsearch depth-0 eviction guard (`cengine.QS_EVICT_MAX = 6` → `set_qs_evict_max`; a P-44 stand-pat store may replace a prior-GENERATION entry only if its depth ≤ 6 — protects the deep cross-move entries that P-14 (+23.5) proved are the engine's most valuable asset; -1 = v40's always-replaceable rule) | Cold-TT ladder PROVABLY unaffected (post-reset old-gen entries are all zeroed depth-0 slots; passes guard-on, no pin needed); warm-play probe @16-bit TT, 4×d12 moves: 1,186,663 → 1,087,957 nodes (−8.3% for the same depth — protected entries deliver more cutoffs) | **PENDING A/B vs Old Engine/40** — eighth 50+0.20 campaign, TT policy is 2-for-2 (P-14 +23.5, P-44 +8.1) 2026-07-11 |
