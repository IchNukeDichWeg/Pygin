...
**Status:** SHIPPED and iterating — all 4 phases done; live = v40, last snapshot Old Engine/40 (C-era ledger ≈ +148 over v31; v37/v38/v40 correctness releases, v39 = +8.86 Phase-2 NPS train; see the version table below). Armed candidate: CB-02 correctness batch #4 (v41-dev row below); FI-08 read a dead null and is DORMANT. **Started:** 2026-07-08 (baseline v30, ~90k NPS). **A/B TC:** 45+0.10 through v36, 50+0.20 from v37 campaigns.
...
| v41-dev | CB-02 correctness batch #4 (`cengine.CB2 = True` → `set_cb2` + driver logic): (a) FB-22 null-move TT store obeys the replacement policy — never clobbers deeper entries, keeps a same-key entry's move; (b) FI-27.1 qsearch 50-move rule (hmc ≥ 100, in-check plays on, hygiene-gated order preserved); (c) FI-24c deep null cutoffs (depth ≥ 10) verified with a reduced NO-NULL re-search (`g_no_null` suppresses nulls in the verification subtree — zugzwang/fortress insurance); (d) FB-23 root fail-high adoption/promotion across aspiration calls (v30's `_partial_root_move` rule, finally ported) | OFF = v40 node-exact (ladder-pinned via the CB2 class attr — it gates BOTH the C toggle and the driver's re-search ordering); shipped default diverges (562,363 → 828,672 @d12 — the verification re-searches cost nodes, the A/B prices the safety); mate-suite sanity clean both configs (4/4 mates found, PVs replay to checkmate) | **PENDING A/B vs Old Engine/40** — ninth 50+0.20 campaign, KEEP-ON-NULL (correctness; PV-02/CB-01/EP-01 precedent) 2026-07-11 |
