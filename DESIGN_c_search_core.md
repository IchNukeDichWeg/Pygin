...
| queued | PV-02 exact PV (`set_pv_exact`, default OFF; PV nodes skip the TT cutoff/bound-narrowing block — the standard strong-engine rule; the TT move still orders) | Verified on a failing matetrack FEN: C PV goes 1 move → full 13-ply mate PV ending in checkmate; d12 3-FEN probe −20% nodes (tree reshapes, sign unknown, no blowup) | **QUEUED as the A/B slot AFTER P-47** 2026-07-10 — matetrack Bad-PVs (~60%) only drop when this lands; confirm ⇒ default ON + matetrack re-baseline |
