...
| v36-dev | P-45 TT prefetch (`set_prefetch`, default on; after `apply_move` compute the child's key and `__builtin_prefetch` its TT line, hiding the probe's DRAM stall behind the in-check test) | Node-identical by construction (ladder passes natively). **Local Apple-Silicon bench: NULL (−0.7% median time-to-depth over 14 alternating reps)** — big SLC + OoO already hide the latency. Server x86 bench pending: smaller caches + 200 contending engines may pay differently. **Gate: bench on the server first; only A/B (vs Old Engine/35) if it shows a real % there, else revert** | PENDING (server bench) 2026-07-10 |
