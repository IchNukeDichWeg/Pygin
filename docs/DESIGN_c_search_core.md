...
**Status:** SHIPPED and iterating -- all 4 phases done; live = **v59**, snapshot Old Engine/59 (C-era ledger ≈ **+338** over v31 -- **v59 ARMS FI-106 LAZY NNUE EVAL (LAZY_NNUE True): skip the net where a cheap bound already decides the node. Isolated for the first time -- same v4 net both sides, the toggle the only variable, vs Old Engine/58 on the corrected (post-fc82cb7) harness: GSPRT[0,4] LLR +2.950 ACCEPT at 2,264 pooled pairs, TIMED 50+0.5 x86, ptnml 69/509/975/595/116, pooled 51.99% -> +13.84 ± 6.4 (stopped early; magnitude bound-biased). W/D/L 1,292/2,138/1,107 over 4,537 games. Bench 1,074,820 -> 1,214,534: +13% nodes for +2.2% NPS on the d11 bench, and on the clock the trade pays. The FIRST release measured as exactly the config that ships -- v58's +19.11 was measured on engine_nnue_v4 (lazy ON) while the release ran lazy OFF, found 2026-08-09** -- **v58 ARMS THE NNUE NET: the first HCE/NNUE hybrid and the first net that pays. engine_nnue_v4 vs Old Engine/57, TIMED 50+0.20 on x86, GSPRT[0,4] LLR +2.950 ACCEPT H1 at 1,702 pairs: +19.11 ± 7.8 over 3,404 games, ptnml 71/358/691/477/105, ratio 1.36. Bench 1,145,629 -> 1,074,820 with ~30% of NPS handed to the SIMD tail, so the +19 is NET of the speed it costs. THE LESSON: v3 read +0.52 ± 6.8 on this same instrument and v4 changed neither the dataset nor a single dimension -- a cosine LR schedule instead of a flat one took held-out val 0.074417 -> 0.066663, and that alone is the +19. The net was never the problem; the training was. The 40-epoch run also settled the next question: val plateaued at epoch 4 while train fell another 22%, so the net is DATA-limited, and with labels still coming from 5,000-node searches while the engine plays at 1.75M (FI-98 priced that at -0.580%), label DEPTH is the next lever, not volume. OWED: an arm64 confirmation, since v3 read +5.70 ± 4.6 there against +0.52 here** -- **v57 is the LAST PURE-HCE release -- **v57 is the LAST PURE-HCE release; from here the engine is an HCE/NNUE hybrid. Node-identical to v56 (bench 1,145,629), host layer only, no A/B slot, ledger unmoved: the ponderhit soft-stop closes ponder's documented v1 deviation (1.666s → 0.686s, ratio 0.412 vs the designed 0.40), the P-35/U-06 neighbourhood is exposed over UCI for zero-rebuild sweeps, and cuci no longer overwrites the engine's own soft_stop_frac with a hardcoded 0.55 -- a bug that would have made every future soft-stop tune work in match.py and vanish under UCI** -- **v56 CONFIRMED +4.11 ±4.2 @21,806 games (`--nodes 1.75M`, GSPRT[0,4] LLR +2.971 ACCEPT): FI-107 ProbCut, the fail-high forward-pruning half Pygin never had, cutting 21.6% of nodes at fixed depth. FOURTH SPRT accept and the first pruning MECHANISM to pay after the search lane was called exhausted -- the parameter space was exhausted, the mechanism space was not. It also cost the EBF gate its credibility for constant-factor changes: `bench/ebf.py` said ABANDON (+0.91%) because it reads the tree's SLOPE, and a constant factor is invisible to it by construction** -- **v55 CONFIRMED +9.66 ±8.2 @6,874 games (TIMED 50+0.20, GSPRT[0,4] LLR +2.946 ACCEPT, stopped early): a node-identical SPEED pair (FI-11 pin-aware legality + FI-42 eval accumulator, +8.3% NPS on x86 / +13.5% on arm64, bench signature 1,461,732 unchanged) and by this file's own P-22 lesson a speed change owes a TIMED A/B before its Elo may be banked -- and it must be TIMED, since a fixed-node instrument reads exactly zero for a change that buys no new nodes. **CALIBRATION: +8.34% NPS -> +9.66 Elo = ~1.16 Elo per 1% NPS**, the low end of the 1-2.7 band**; v54 +31.20 PST retune, v53 +37.52 Texel retune; **v53 +37.52 -- the Texel eval retune, the largest single release in the ledger and the EVAL lane's first win, landed immediately after the search lane was declared exhausted; no C change, 44 scalars refitted in engine.py**; v44 +13.31, v45 +13.52, v46 +5.94 (96 MB TT), v47 +3.16 (192 MB TT), v48 +4.73 (FI-30 qsearch TT-quality), v49 +0.97 (FI-29 cuckoo, null kept), v50 +1.60 (FI-53 rule50 TT guard + FI-54 depth-independent mate handling, null KEPT as correctness -- seventh+eighth of the class), v51 +11.12 (FI-56 root-move LMR -- SECOND SPRT accept; first root reductions ever), v52 +6.63 (FI-24(a)+(b) null-move refinements -- THIRD SPRT accept, and the first verdict confirmed on the `--nodes` instrument); see the version table below). 2026-07-18 also resolved: FI-50/51/52 qsearch-TT batch NULL −0.28 reverted (24th), FI-48 flag-aware replacement CLOSED DEAD-GATE pre-A/B (~0.001% engagement, no slot), FI-49 fail-high tightening REJECTED −3.65 (25th, +28% node cost never paid -- dormant do-not-retry). FI-23 history-driven quiet pruning REJECTED 2026-07-16 (twenty-first campaign, −5.23±7.1 @9,243 games, SPRT ACCEPT H0 stopped early -- a real negative; HIST_PRUNE=0 dormant, do-not-retry; shallow-prune vein 0-for-2 with FI-18). FI-09 bundle NULL 2026-07-14 (twentieth campaign, +0.69±6.8, reverted, dormant) closed the time-policy vein alongside the soft_stop_frac 0.60 null. v48 = v47 + FI-30 qsearch TT-quality batch: **CONFIRMED** 2026-07-16 (twenty-second campaign vs Old Engine/47, four pooled tranches = 21,605 games: **+4.73 ±3.19**, pair ratio 1.08, pooled GSPRT[0,4] LLR **+3.475** > +2.944 -- the C era's first sequential-test ACCEPT; a premature revert at the 10k budget cap was walked back and the test ran to its own stopping rule -- sequential tests are never capped at a fixed budget again). v49 = v48 + FI-29 cuckoo upcoming-repetition: **KEPT-ON-NULL** 2026-07-17 (twenty-third campaign vs Old Engine/48, 10,000 games: **+0.97 ±6.8**, pair ratio 1.02, GSPRT[0,4] LLR −0.19 -- the pre-registered correctness-class rule ships the null, sixth of its class). Armed: none. 2026-07-21 also closed the P-26 parameter sweep (LMR_DIV 170 screen-null; NULL_BASE/LMP/futility probes anti-thesis, RFP flat -- the selectivity PARAMETERS are on a plateau, the levers are code) and shipped real UCI pondering (host layer). Next campaigns run vs Old Engine/53 on SUBSET_SEED 53; queue = the `texel.py --with-dormant` candidate (the five BUILT-but-OFF eval terms, weights fitted for the first time), a second Texel pass against the new baseline, and the NPS bench-class train (FI-67/68-75) in parallel, with NNUE still the eval lane's endgame. FI-12 screen-null closed the history vein 0-for-4. FI-06 +2.26, FI-18 −1.25, FI-04 +2.15 all null (dormant). **Started:** 2026-07-08 (baseline v30, ~90k NPS). **A/B TC:** 45+0.10 through v36, 50+0.20 from v37 campaigns.
...
| **v56** | **FI-107 ProbCut** (`PROBCUT_MARGIN`/`PROBCUT_DEPTH`/`PROBCUT_RED` → `set_probcut`): at a non-PV node past depth 5, not in check, with |beta| below the mate threshold, a qsearch filters each capture at `beta + 200` and a real `depth − 4` negamax CONFIRMS before anything is returned. A deeper TT bound vetoes the probe. Nothing is ever cut on a static score -- that is what sank FI-18 (−1.25) and FI-23 (−5.23); the two-stage verify is a different mechanism, not a wider-margin retry. Returns `v − (pbeta − beta)`, a valid fail-high without the inflated probe score. abi 34→35 (four exports had landed without a bump) | off = v55 node-exact (bench 1,461,732 byte-exact); armed bench **1,145,629 (−21.6%)**; margin sweep 150/200/300 → −14.5/−21.6/−7.7% (non-monotone: a wider margin cuts less but still pays for the probes); NPS cost 2.4%, charged by the `--nodes` calibration; v56 ladder re-measured | **CONFIRMED → v56** 2026-07-30 (vs `cengine` baseline, `--nodes 1.75M`): screen 3,000 pairs LLR +0.464 → tranche 1 pooled +1.128 → tranche 2 pooled +2.190 → **21,806 games +4.11 ±4.2** (50.59%, ptnml 530/2606/4452/2706/609, GSPRT[0,4] LLR **+2.971 ACCEPT**, stopped early) -- the fourth SPRT accept, monotone toward accept across three independent samples. **Two instrument lessons:** the +15 screen gate is TRIAGE, not the ship threshold (this read +2.90 on a 2k screen and was wrongly closed as null -- a true +3 sits BETWEEN the [0,4] bounds, exactly what a screen cannot resolve); and `match.py` scores its pentanomial from ENGINE 1, so the CANDIDATE GOES IN SLOT 1 -- with it in slot 2 the printed LLR answers the mirror question, which is not the same number sign-flipped, and pooled it 'decided' ACCEPT H0 on a claim nobody had made. **The EBF gate (FI-108) said ABANDON and was WRONG**: it reads tree SLOPE, and ProbCut is a constant factor (−21.6% nodes, flat growth). The gate keeps its value only where a real slope penalty exists -- FI-105 (−1.51, screened) and FI-106 razoring (+3.7% EBF) were both correctly killed by it. **TIMED CROSS-CHECK, the pre-registered obligation owed since FI-24, DISCHARGED 2026-07-30**: v56 vs Old Engine/55 on a CLOCK (50+0.20, SUBSET_SEED 56, fresh openings), 5,924 games: **+11.44 ±6.9** (51.65%, ptnml 124/682/1216/755/185, nElo +17.07, GSPRT[0,4] LLR **+2.953 ACCEPT**, stopped early). The check existed to catch `--nodes` OVERPAYING a speed-flavoured change through its NPS calibration; it does the opposite -- the fixed-node instrument reads CONSERVATIVE (+4.11 vs +11.44). Every verdict since FI-24 rests on safer ground. The timed figure is what the ledger banks, per this table's own 50+0.20 convention; note it prices v55→v56 AS A WHOLE (ProbCut + FB-48's stalemate contempt), not ProbCut alone. Snapshotted Old Engine/56 |
| **v55** | **Node-identical SPEED pair, no A/B slot spent.** FI-11 pin-aware legality (`legal_ctx` builds a pinned mask per node from empty-board sniper rays + `INBETWEEN_BITBOARDS`; out of check an unpinned non-king non-ep mover is legal by construction -- king moves, en passant and in-check nodes keep the full `legal()` scan) + FI-42 the (mg,eg,phase) accumulator on Board (`apply_move` maintains the tapered material+PST sum on exactly the squares FI-01's Zobrist XORs touch, so `eval_white`'s 12 ctz loops over up to 32 pieces are gone; Board 88 → 104 bytes) | **bench signature 1,461,732 UNCHANGED**; perft `--deep` 1,494,874,546 nodes ALL PASS; accumulator differential 0 mismatches over ~117M nodes; ladder d14 node-exact. NPS on `bench/nps13.py` with the new `--repeat` control: **+8.34/+8.35/+8.30% on x86** (Gold 6330 pinned+idle, 48/48 rounds, between-run spread 0.05) and **+13.5% on arm64**. FI-42 is ~+8 points on BOTH; FI-11 is +5% on arm64 and a WASH on x86 | **CONFIRMED → v55** 2026-07-25: **+9.66 ±8.2 @ 6,874 games** TIMED 50+0.20 vs Old Engine/54 (51.39%, ptnml 133/783/1460/866/188, ratio 1.15, norm +14.98 (FB-54 scale; +21.18 pre-fix), GSPRT[0,4] LLR **+2.946 ACCEPT**, stopped early) -- the sixth SPRT accept and the first release measured on pure time-to-depth. **~1.16 Elo per 1% NPS** -- the conversion that now prices every bench item. Per this table's own P-22 lesson, a node-identical speed change still owes a TIMED A/B, and it must be timed: the `--nodes` instrument reads exactly zero for a change that buys no new nodes. Prior conversions ~1 (v39) / ~2.7 (v44) had left the estimate 4x wide at +8 to +22; the measurement landed at the bottom. New wrinkle the ledger has never faced: the gain is ARCHITECTURE-DEPENDENT, so the Elo depends on which machine plays. Snapshotted Old Engine/55 |
| -- | **texel `--with-dormant`**: the five BUILT-but-OFF eval terms (outpost, space, phalanx, storm, king shelter) enabled together, their 11 weights added to the tuned set, and all 44 live scalars refitted on 5,000,000 positions | held-out loss **−0.32%** vs v53 (0.090098 → 0.089812, same K=1.1, measured per config in its own process); the tuner left all 11 dormant weights at their hand-guessed defaults (verified live, not a dead-parameter bug: +40cp moves the C eval on 2–587 of 724 probes); paired NPS cost ~0.9% | **REJECTED 2026-07-23** (thirty-fourth campaign vs Old Engine/53, `--nodes 1.75M`, two servers): **−8.14 ±5.5 over 7,942 games** (48.83%, ptnml 240/1015/1594/935/187, pair ratio 0.89, LLR −1.708), stopped early -- both halves negative and consistent (−5.57 / −10.37). **The slot should never have been spent**: king shelter carries a 10k C-core A/B at −4.27 ±6.8 *and an explicit "do not re-try"*, outpost a 10k null at −0.90, space −9 and storm −5 in the Python era. The flag's premise -- "rejected on hand-guessed weights against a depth-8 engine" -- was false for half of them. **Held-out loss and Elo pointed in OPPOSITE directions**: −0.32% loss, −8.14 Elo. Isolating the halves afterwards: the 44-param retune ALONE is worth +0.07% held-out (0.090039 vs v53's 0.090098) -- noise -- so essentially the whole loss gain came from the five dormant terms, i.e. the terms that cost the Elo IMPROVED the proxy by +0.25%. The loss proxy is calibrated on exactly one point (v53) and does not survive extrapolation to small magnitudes |
